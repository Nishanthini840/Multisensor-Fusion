...
Simplicity-First 3-Step Streamlit Interface - Modern Minimalist Light Theme.
"""
import os
import sys
import pandas as pd
...
with col_output:
    ...
    <div style="background: #F8FAFC; border: 1px solid #E2E8F0; border-left: 4px solid {color}; border-radius: 4px; padding: 12px;">
        ...
        <div style="font-size: 0.75rem; color: #64748B; margin-top: 6px;">Timeframe: <strong style="color: #0F172A;">{recommendation.get('timeframe', 'Routine')}</strong> | Priority: <strong style="color: {color};">{recommendation.get('priority', 'LOW')}</strong></div>
    </div>
    """, unsafe_allow_html=True)
